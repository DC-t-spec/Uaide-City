...
func _ensure_environment_setup() -> void:
	...
	environment.glow_strength = 0.28
	environment.glow_bloom = 0.03
	environment.glow_hdr_threshold = 1.28
...
func _update_visual_phase(time_float: float) -> void:
	...
	if time_float >= 5.0 and time_float < 8.0:
		...
		_apply_visual(
			...
			Color(0.30, 0.34, 0.60).lerp(Color(0.44, 0.66, 0.95), t),
	...
	if time_float >= 8.0 and time_float < 15.0:
		...
		_apply_visual(
			...
			Color(0.36, 0.61, 0.95).lerp(Color(0.30, 0.56, 0.92), midday_strength),
			Color(0.58, 0.67, 0.79).lerp(Color(0.66, 0.73, 0.84), midday_strength),
			Color(0.72, 0.81, 0.93).lerp(Color(0.78, 0.86, 0.96), midday_strength),
			lerp(0.78, 0.95, midday_strength),
			0.004
		)
		return
